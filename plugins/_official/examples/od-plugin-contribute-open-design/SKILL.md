...
description: Open a pull request adding a local Clean Design plugin to the Clean Design community catalog using gh CLI.
triggers:
  - contribute plugin
  - open design pr
  - github pull request
...
# Contribute Plugin to Clean Design

Use this workflow when the active project contains a copied plugin folder and the user wants to propose it for the Clean Design community catalog.
...
Keep the pull request focused. Do not modify unrelated Clean Design files unless a manifest validation issue requires a tiny supporting change.
