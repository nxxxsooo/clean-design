...
Give this file to a coding agent when you want it to create or improve an Clean Design plugin.

## Mission

Create a portable Clean Design plugin that can:

1. Run as a normal Agent Skill through `SKILL.md`.
2. Install into Clean Design through `open-design.json`.
3. Be validated locally.
4. Be published as an independent open source repo or submitted as a PR to Clean Design.
...
- `plugins/spec/PUBLISHING-REGISTRIES.md` when the user asks to publish outside Clean Design
...
3. Keep the `SKILL.md` portable. It may mention Clean Design behavior, but the core workflow must still make sense in any Agent Skills compatible agent.
