...
Clean Design plugins are intentionally shaped so one folder can travel across multiple agent ecosystems. The safest publishing model is:

1. Keep the source of truth in a public GitHub repository or in an Clean Design PR.
2. Keep `SKILL.md` portable and registry-friendly.
3. Add `open-design.json` as the Clean Design sidecar.
...
2. Push a public GitHub repository or open an Clean Design PR.
3. Add README install instructions for Clean Design and generic Agent Skills clients.
...
| --- | --- | --- | --- |
| Clean Design | OD marketplace, composer chips, pipelines, GenUI, artifact provenance | `SKILL.md` + `open-design.json` | Open a PR to Clean Design or publish a marketplace index entry pointing to the plugin repo. |
...
For Clean Design plugin authors:
...
Use this path for normal Clean Design plugins because they are centered on `SKILL.md`.
...
- Link back to the canonical GitHub repo and Clean Design PR.
