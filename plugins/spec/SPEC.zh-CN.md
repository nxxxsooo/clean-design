...
这是可移植 Clean Design 插件的精简契约。完整产品规范仍以 `docs/plugins-spec.zh-CN.md` 为准；本文档面向贡献者和外部编码 agent。
...
## 2. 增强版 Clean Design 插件

当插件需要出现在 Clean Design marketplace 卡片或 starter 中时，添加 `open-design.json`：
...
- `name` 是稳定插件 id。
- `specVersion` 是此 manifest 遵循的 Clean Design 插件规范版本。除非 schema 升级，否则使用当前规范包的值（`1.0.0`）。
...
外部 registry 分发策略见 [`PUBLISHING-REGISTRIES.zh-CN.md`](PUBLISHING-REGISTRIES.zh-CN.md)。简言之：把 GitHub 或 Clean Design PR 作为 source of truth，让文件夹能作为通用 `SKILL.md` skill 安装；本地验证通过后，再发布或登记到 skills.sh、ClawHub 或其他 registry。
