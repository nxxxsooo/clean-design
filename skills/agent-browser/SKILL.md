...
description: |
  ...
  Clean Design browser-tab context, testing web apps, dogfooding Clean Design
  previews, QA, bug hunts, or reviewing app quality. Prefer local Clean Design
...
Use `agent-browser` for local Clean Design preview validation: inspect rendered
...
For selected Clean Design browser tabs and browser-use/browser-harness-style
...
5. If the selected Clean Design context only provided a URL/title and no browser
...
Do not run Clean Design's own daemon CLI as a browser automation tool. Commands
...
With the Clean Design preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `Clean Design`, current URL under `127.0.0.1:17573`,
visible Clean Design UI text in the snapshot, and a screenshot at
