...
name: ui-ux-pro-max
description: |
  Catalog-only UI/UX Pro Max entry. The full upstream templates, data, and search workflow are not bundled in Clean Design.
triggers:
  - "ui ux patterns"
  - "design patterns"
  - "ux heuristics"
...
## Current Clean Design scope

Clean Design currently ships this entry as discovery metadata only. If this `SKILL.md`
...
This catalogue entry advertises the skill in Clean Design so the agent
...
If those upstream files are not installed, explain that Clean Design only has the
catalog entry for this skill and ask whether to continue with Clean Design's
