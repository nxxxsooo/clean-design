...
macDescribe('packaged mac runtime smoke', () => {
  ...
  test('builds, installs, starts, inspects, stops, and uninstalls the mac artifact', async () => {
    const report = await createPackagedSmokeReport('mac');
    let passed = false;
    try {
      const build = await runToolsPackJson<MacBuildResult>('build', ['--to', 'all']);
      expect(build.to).toBe('all');
      expect(build.dmgPath).not.toBeNull();
      expectPathInside(build.appPath, outputNamespaceRoot);
      expectPathInside(build.dmgPath ?? '', join(outputNamespaceRoot, 'dmg'));
      expect(build.outputRoot).toBe(outputNamespaceRoot);
      expect(build.runtimeNamespaceRoot).toBe(runtimeNamespaceRoot);
      ...
      expect(inspect.status?.url).toMatch(/^(cleandesign:\/\/app\/|http:\/\/127\.0\.0\.1:\d+\/)/);

      const value = assertHealthEvalValue(inspect.eval?.value);
      expect(value.href).toMatch(/^(cleandesign:\/\/app\/|http:\/\/127\.0\.0\.1:\d+\/)/);
      expect(value.status).toBe(200);
      expect(value.health.ok).toBe(true);
      expect(value.health.version).toEqual(expect.any(String));
      expect(inspect.launcher.exists).toBe(true);
      expect(inspect.launcher.active).toEqual(inspect.launcher.lastSuccessful);
      expect(inspect.launcher.active?.generation).toBe(0);
      expect(inspect.launcher.attempt).toBeNull();
...
desktopMacDescribe('mac desktop settings smoke', () => {
  ...
  test('keeps provider endpoint fields coherent when switching API protocols', async () => {
    await seedDesktopConfig(desktop, {
      mode: 'api',
      apiKey: 'sk-test',
