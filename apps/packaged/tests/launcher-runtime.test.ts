...
describe("resolvePackagedLauncherRuntime", () => {
  ...
  it("uses the active launcher payload when runtime state and payload manifest are valid", async () => {
    ...
    try {
      ...
      const resourcesPath = join(versionPaths.payloadRoot, "Clean Design Beta.app", "Contents", "Resources");
      ...
      await mkdir(join(versionPaths.payloadRoot, "Clean Design Beta.app", "Contents", "MacOS"), { recursive: true });
      ...
      await writeFile(
        ...
        `${JSON.stringify({
          channel: "beta",
          launchPath: "/Applications/Clean Design Beta.app",
          namespace: config.namespace,
          schemaVersion: LAUNCHER_SCHEMA_VERSION,
        })}\n`,
      );
      ...
      expect(runtime.electronNodeCommand).toBeNull();
      expect(runtime.installedLaunchPath).toBe("/Applications/Clean Design Beta.app");
  ...
  it("uses the Windows payload executable as the Electron-as-Node command when payload Node is absent", async () => {
    ...
    try {
      ...
      const payloadExePath = join(versionPaths.versionRoot, "payload", "Clean Design.exe");
