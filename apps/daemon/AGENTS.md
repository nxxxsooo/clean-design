...
- `agents.ts`, `run-*`, `agent-*`, `*-diagnostics.ts` -> usually `src/runtimes/` or a future `src/runs/` folder, depending on ownership. (`claude-stream.ts`, `json-event-stream.ts`, and `runs.ts` already live under `src/runtimes/`.)
