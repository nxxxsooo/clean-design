...
test('codex thread.started surfaces the thread id as the status sessionId', () => {
  const { events, handler } = collectEvents('codex');
  handler.feed(
...
test('codex thread.started without a thread id reports a null sessionId', () => {
  const { events, handler } = collectEvents('codex');
