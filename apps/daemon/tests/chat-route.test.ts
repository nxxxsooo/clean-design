...
describe('/api/chat', () => {
  ...
  async function seedBundledPlugin(pluginId: string, fixtureDir: string): Promise<void> {
    if (!process.env.OD_DATA_DIR) throw new Error('OD_DATA_DIR is required for bundled plugin fixtures');
...
`,
  ...
  it('preserves plugin-local and composed @-mention skills in plugin-bound runs', async () => {
    ...
    // Clean Design removed the hosted install endpoint; seed the plugin
    // registry locally the way bundled plugins are provisioned.
    await seedBundledPlugin(pluginId, pluginFixtureDir);
