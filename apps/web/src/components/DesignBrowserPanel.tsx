...
export function browserUsePrompt(action: BrowserUseAction, context: BrowserUsePromptContext = {}): string {
  ...
  return [
    '@agent-browser',
    '',
    'Use the selected Clean Design Browser tab as the bound target.',
    'Browser tab context:',
    `- tab: ${tabLabel}`,
    `- title: ${title}`,
    `- url: ${url}`,
...
// Slugs a reference site URL into the translation key used for its detail:
// hostname minus the TLD, with non-alphanumerics folded into underscores
// (`land-book.com` → `land_book`, `fonts.google.com` → `fonts_google`).
