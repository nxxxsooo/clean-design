...
$sourceInstaller = Join-Path $builderDir "Clean Design-${env:TOOLS_PACK_NAMESPACE}-setup.exe"
$sourceZip = Join-Path $builderDir "Clean Design-${env:TOOLS_PACK_NAMESPACE}-portable.zip"
...
$releaseNotes = if ([string]::IsNullOrWhiteSpace($env:RELEASE_NOTES)) {
  "Clean Design ${env:RELEASE_VERSION}$assetSuffix"
} else {
  $env:RELEASE_NOTES
}
