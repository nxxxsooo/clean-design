...
async function runFixture(options: {
  ...
  const resourcesRoot = platformName === "darwin"
    ? join(appOutDir, "Clean Design.app", "Contents", "Resources")
    : join(appOutDir, "resources");
  const appPath = join(appOutDir, "Clean Design.app");
  ...
  if (platformName === "darwin" && options.writeMacCodeBundleFixture) {
    ...
    await mkdir(join(frameworksRoot, "Clean Design Helper.app"), { recursive: true });
  }
  ...
  try {
    await runWebStandaloneAfterPack({
      ...
      packager: { appInfo: { productFilename: "Clean Design" } },
    });
  } catch (error) {
    ...
    try {
      ...
      expect(signedTargets).toEqual(
        expect.arrayContaining([
          ...
          expect.stringMatching(/Clean Design Helper\.app$/),
          expect.stringMatching(/Clean Design\.app$/),
        ]),
      );
