...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Clean Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Clean Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Clean Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Clean Design", "Clean Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Clean Design-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Clean Design-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Clean Design-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Clean Design", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Clean Design.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "Clean Design.exe"))).resolves.toMatchObject({
