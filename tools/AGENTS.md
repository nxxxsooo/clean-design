...
- `tools/pack` provides `@open-design/tools-pack` and the `tools-pack` bin. Clean Design uses its local Apple Silicon build, install, start, stop, logs, uninstall, cleanup, list, and reset commands.
...
- Keep `tools-pack` focused on local packaging and runtime control. Clean Design has no updater or remote release-publication path.
